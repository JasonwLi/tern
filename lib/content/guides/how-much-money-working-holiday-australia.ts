import type { Article } from "../types";

export const article: Article = {
  slug: "how-much-money-working-holiday-australia",
  kind: "guide",
  date: "2026-05-29",
  emoji: "💵",
  accent: "bg-lime-300",
  readMinutes: 7,
  related: ["proof-of-funds-working-holiday-visa", "first-week-money-checklist-australia"],
  sources: [
    {
      label: "Australian Government — Working Holiday visa (subclass 417)",
      url: "https://immi.homeaffairs.gov.au/visas/getting-a-visa/visa-listing/work-holiday-417/first-working-holiday-417",
    },
    {
      label: "Australian Government — Home Affairs visa pricing",
      url: "https://immi.homeaffairs.gov.au/visas/getting-a-visa/fees-and-charges",
    },
  ],
  i18n: {
    en: {
      title: "How much money for a working holiday in Australia?",
      description:
        "Exact figures for the Australia working holiday visa fee, AUD 5,000 proof of funds, first-month budget breakdown, and how much to save before you fly.",
      excerpt:
        "The visa costs AUD 670. You'll need AUD 5,000 in the bank plus an onward flight. Here's what your first month in Australia actually costs — and how to land with enough buffer.",
      sections: [
        {
          paragraphs: [
            "Australia's working holiday visa (subclass 417) is one of the most popular gaps in the world — but before you book flights, you need to know exactly how much money to have in the bank. Between the visa fee, the proof-of-funds requirement, and the real cost of your first month, the number is higher than most people expect.",
          ],
        },
        {
          heading: "Visa application fee",
          paragraphs: [
            "The current application fee for the subclass 417 Working Holiday visa is approximately AUD 670 (as of mid-2026 — always verify the current amount on the Department of Home Affairs website before you apply, as fees are reviewed annually). The fee is non-refundable and must be paid online when you lodge your application through ImmiAccount. There is no separate biometrics or health exam fee for most nationalities applying for a first 417 visa.",
          ],
        },
        {
          heading: "Proof of funds: AUD 5,000 + onward ticket",
          paragraphs: [
            "The Department of Home Affairs requires working holiday visa holders to have sufficient funds to support themselves throughout their stay. The widely-cited benchmark is AUD 5,000 in accessible savings, plus either a return or onward flight ticket — or the funds to buy one. For a full breakdown of what counts and how to show it, see our guide to [proof of funds for a working holiday visa](~/blog/proof-of-funds-working-holiday-visa).",
            "You are not typically asked to show a bank statement at the airport, but immigration officers can request it. More practically, the AUD 5,000 benchmark is what your bank account should show if you want to be ready for any scenario, including a landlord or employer who asks for proof of funds.",
          ],
          bullets: [
            "AUD 5,000 in a savings or transaction account — liquid funds only; credit card limits do not count",
            "Return or onward flight booking, or equivalent funds to purchase one",
            "Bank statement dated within the last 3 months showing your name and balance",
            "If you carry or transfer AUD 10,000 or more, you must declare it on your incoming passenger card",
          ],
        },
        {
          heading: "First-month budget breakdown",
          paragraphs: [
            "Your first month in Australia is typically your most expensive — you're paying for accommodation before you've found a share house, eating out before you have a kitchen, and buying gear you didn't pack. Here's a realistic weekly and monthly breakdown for someone based in a major city like Sydney or Melbourne.",
          ],
          bullets: [
            "Hostel dorm bed: AUD 180–320 per week (varies significantly by city and season)",
            "Food and groceries: AUD 80–150 per week (cooking in hostel kitchen cuts costs sharply)",
            "Local SIM card: AUD 30–50 for the first month (prepaid plans from Telstra, Optus, or Vodafone)",
            "Public transport: AUD 50–100 per week depending on how much you're moving around",
            "Rental bond: AUD 1,000–2,000 if you move into a share house (typically 4 weeks' rent in advance plus bond)",
            "Incidentals (toiletries, clothing, entertainment): AUD 100–200 per month",
          ],
        },
        {
          heading: "Buffer until your first paycheck",
          paragraphs: [
            "Most Australian employers pay weekly or fortnightly. If you start work in week one, you could receive your first pay within 7–14 days — but payroll processing delays and the time needed to open a bank account and get a Tax File Number (TFN) mean two to four weeks of zero income is a realistic expectation.",
            "Add up four weeks of the costs above and you're looking at AUD 1,800–3,000 in living expenses before you earn anything. This is why AUD 5,000 is the minimum — not a comfortable buffer.",
          ],
        },
        {
          heading: "Sample totals: how much to save before you fly",
          paragraphs: [
            "Putting it all together, here are realistic total savings targets before you board the plane.",
          ],
          bullets: [
            "Bare minimum (tight budget, shared hostel, start work fast): AUD 5,000 + return flight",
            "Comfortable buffer (first month covered, small emergency fund): AUD 7,000–8,000",
            "Recommended if you want to travel first or take your time finding work: AUD 10,000+",
            "Remember: these are arrival savings — not counting the visa fee you'll pay before you leave home",
          ],
        },
        {
          heading: "How Tern helps",
          paragraphs: [
            "Tern's proof-of-funds statement generator lets you export a clean, dated summary of your balance to show immigration officers, landlords, or employers — no more hunting for the right bank statement format. You can fund your Tern account from home at the real mid-market rate before you fly, so you land with AUD already in your pocket at zero markup. Once you start work, your salary arrives on payday with no holding delays, and Tern charges no ATM fees so every dollar you withdraw is yours.",
          ],
        },
      ],
      faq: [
        {
          q: "How much money do I need for a working holiday in Australia?",
          a: "The Department of Home Affairs sets the benchmark at AUD 5,000 in accessible funds plus an onward or return flight. On top of that, budget for the AUD 670 visa application fee (paid before you travel) and realistically AUD 1,800–3,000 for your first month of living costs before your first paycheck arrives. A total pre-departure savings target of AUD 7,000–8,000 gives you a comfortable buffer.",
        },
        {
          q: "Do immigration officers actually check proof of funds in Australia?",
          a: "Australian border officers may check, and the Department of Home Affairs can request proof when you apply or on arrival. More commonly, landlords, share-house listings, and some employers will ask to see a bank statement. Having AUD 5,000 visible in a bank account (not a credit limit) is the practical standard you should meet.",
        },
        {
          q: "Can I use my home bank account to show proof of funds?",
          a: "Yes — a bank statement from your home country account showing AUD 5,000 equivalent is acceptable for the visa and border check. However, once you're in Australia you'll need an Australian bank account (with a BSB and account number) for direct deposit from employers. Open one as soon as you arrive and apply for a Tax File Number at the same time.",
        },
      ],
    },
    es: {
      title: "¿Cuánto dinero necesitas para working holiday en Australia?",
      description:
        "Cifras exactas para la tarifa de la visa 417, los AUD 5,000 de fondos requeridos, el presupuesto del primer mes y cuánto ahorrar antes de volar.",
      excerpt:
        "La visa cuesta aproximadamente AUD 670. Necesitas AUD 5,000 en el banco más un vuelo de salida. Esto es lo que realmente cuesta tu primer mes en Australia — y cómo llegar con suficiente colchón.",
      sections: [
        {
          paragraphs: [
            "La visa working holiday de Australia (subclase 417) es uno de los programas más populares del mundo — pero antes de reservar vuelos, tienes que saber exactamente cuánto dinero tener en el banco. Entre la tarifa de la visa, el requisito de fondos mínimos y el costo real del primer mes, el número es más alto de lo que la mayoría espera.",
          ],
        },
        {
          heading: "Tarifa de la solicitud de visa",
          paragraphs: [
            "La tarifa actual de solicitud para la visa Working Holiday subclase 417 es de aproximadamente AUD 670 (a mediados de 2026 — verifica siempre el monto actual en el sitio web del Departamento de Asuntos Internos antes de solicitar, ya que las tarifas se revisan anualmente). La tarifa no es reembolsable y debe pagarse online al presentar la solicitud a través de ImmiAccount. Para la mayoría de las nacionalidades que solicitan una primera visa 417 no hay tasas adicionales de biometría ni examen médico.",
          ],
        },
        {
          heading: "Fondos mínimos: AUD 5,000 + vuelo de salida",
          paragraphs: [
            "El Departamento de Asuntos Internos requiere que los titulares de visa working holiday tengan fondos suficientes para mantenerse durante su estadía. El estándar más citado es AUD 5,000 en ahorros accesibles, más un billete de regreso o vuelo de salida — o fondos para comprarlo. Para saber exactamente qué documentos valen y cómo presentarlos, consulta nuestra guía sobre [comprobante de fondos para la visa working holiday](~/blog/proof-of-funds-working-holiday-visa).",
            "Generalmente no te piden un extracto bancario en el aeropuerto, pero los oficiales de inmigración pueden solicitarlo. En la práctica, los AUD 5,000 son lo que debería mostrar tu cuenta si quieres estar preparado para cualquier escenario, incluyendo un propietario o empleador que te pida comprobante de fondos.",
          ],
          bullets: [
            "AUD 5,000 en una cuenta de ahorro o cuenta corriente — solo fondos líquidos; el límite de la tarjeta de crédito no cuenta",
            "Reserva de vuelo de regreso o de salida, o fondos equivalentes para comprarlo",
            "Extracto bancario de los últimos 3 meses con tu nombre y saldo",
            "Si portas o transfieres AUD 10,000 o más, debes declararlo en tu tarjeta de pasajero entrante",
          ],
        },
        {
          heading: "Desglose del presupuesto del primer mes",
          paragraphs: [
            "Tu primer mes en Australia suele ser el más caro — pagas alojamiento antes de encontrar una casa compartida, comes fuera antes de tener cocina y compras cosas que no trajiste. Acá hay un desglose realista semanal y mensual para alguien en una ciudad grande como Sídney o Melbourne.",
          ],
          bullets: [
            "Cama en dormitorio de hostel: AUD 180–320 por semana (varía según ciudad y temporada)",
            "Comida y víveres: AUD 80–150 por semana (cocinar en la cocina del hostel reduce costos notablemente)",
            "SIM local: AUD 30–50 para el primer mes (planes prepagos de Telstra, Optus o Vodafone)",
            "Transporte público: AUD 50–100 por semana según cuánto te muevas",
            "Depósito de alquiler: AUD 1,000–2,000 si te mudas a una casa compartida (generalmente 4 semanas de alquiler adelantado más depósito)",
            "Gastos varios (higiene, ropa, entretenimiento): AUD 100–200 por mes",
          ],
        },
        {
          heading: "Colchón hasta el primer pago",
          paragraphs: [
            "La mayoría de los empleadores australianos pagan semanal o quincenalmente. Si empiezas a trabajar en la primera semana, puedes recibir tu primer pago en 7 a 14 días — pero los retrasos en la nómina y el tiempo necesario para abrir una cuenta bancaria y obtener un Tax File Number (TFN) hacen que dos a cuatro semanas sin ingresos sea una expectativa realista.",
            "Suma cuatro semanas de los costos anteriores y estás mirando AUD 1,800–3,000 en gastos de vida antes de ganar nada. Por eso AUD 5,000 es el mínimo — no un colchón cómodo.",
          ],
        },
        {
          heading: "Totales de ejemplo: cuánto ahorrar antes de volar",
          paragraphs: [
            "Sumando todo, estos son los objetivos de ahorro total realistas antes de subir al avión.",
          ],
          bullets: [
            "Mínimo indispensable (presupuesto ajustado, hostel compartido, empezar a trabajar rápido): AUD 5,000 + vuelo de regreso",
            "Colchón cómodo (primer mes cubierto, pequeño fondo de emergencia): AUD 7,000–8,000",
            "Recomendado si quieres viajar primero o tomarte tu tiempo para encontrar trabajo: AUD 10,000+",
            "Recordá: estos son ahorros al llegar — sin contar la tarifa de visa que pagarás antes de salir de tu país",
          ],
        },
        {
          heading: "Cómo te ayuda Tern",
          paragraphs: [
            "El generador de comprobantes de fondos de Tern te permite exportar un resumen limpio y con fecha de tu saldo para mostrárselo a oficiales de inmigración, propietarios o empleadores — sin más búsquedas del formato correcto de extracto bancario. Puedes fondear tu cuenta Tern desde tu país al tipo de cambio real antes de volar, así llegas con AUD ya en el bolsillo sin recargo. Una vez que empiezas a trabajar, tu sueldo llega el día de pago sin demoras y Tern no cobra comisiones de cajero — cada dólar que retiras es tuyo.",
          ],
        },
      ],
      faq: [
        {
          q: "¿Cuánto dinero necesito para una working holiday en Australia?",
          a: "El Departamento de Asuntos Internos fija el estándar en AUD 5,000 en fondos accesibles más un vuelo de salida o regreso. Además de eso, presupuesta la tarifa de solicitud de visa de aproximadamente AUD 670 (que pagas antes de viajar) y de manera realista AUD 1,800–3,000 para tu primer mes de gastos de vida antes de recibir tu primer pago. Un objetivo de ahorro total antes de partir de AUD 7,000–8,000 te da un colchón cómodo.",
        },
        {
          q: "¿Los oficiales de inmigración realmente verifican los fondos en Australia?",
          a: "Los oficiales de frontera australianos pueden verificar, y el Departamento de Asuntos Internos puede solicitar comprobante al momento de la solicitud o al llegar. Más frecuentemente, propietarios, anuncios de casas compartidas y algunos empleadores te pedirán un extracto bancario. El estándar práctico que debes cumplir es tener AUD 5,000 visibles en una cuenta bancaria (no un límite de crédito).",
        },
        {
          q: "¿Puedo usar mi cuenta bancaria del país de origen para mostrar fondos?",
          a: "Sí — un extracto bancario de tu cuenta en el extranjero que muestre el equivalente a AUD 5,000 es aceptable para la visa y el control fronterizo. Sin embargo, una vez en Australia necesitarás una cuenta bancaria australiana (con BSB y número de cuenta) para que los empleadores te hagan depósito directo. Abre una en cuanto llegues y solicita tu Tax File Number al mismo tiempo.",
        },
      ],
    },
    ja: {
      title: "オーストラリアのワーキングホリデーに必要なお金は？",
      description:
        "417ビザの申請費用、AUD 5,000の資金証明、初月の予算内訳、渡航前に貯めるべき金額を正確に解説します。",
      excerpt:
        "ビザはAUD 670。銀行にAUD 5,000と帰国便が必要です。オーストラリアでの初月の実際のコストと、十分な余裕を持って到着するための方法を紹介します。",
      sections: [
        {
          paragraphs: [
            "オーストラリアのワーキングホリデービザ（サブクラス417）は世界で最も人気のある制度の一つですが、フライトを予約する前に、銀行にいくら必要かを正確に把握しておく必要があります。ビザ費用、資金証明の要件、初月の実際のコストを合わせると、多くの人が想定するよりも大きな金額になります。",
          ],
        },
        {
          heading: "ビザ申請費用",
          paragraphs: [
            "サブクラス417ワーキングホリデービザの現在の申請費用はAUD 670程度です（2026年中頃時点 — 費用は毎年見直されるため、申請前に必ず移民・国境保護省のウェブサイトで最新の金額を確認してください）。申請費用は返金不可で、ImmiAccountを通じてオンラインで申請する際に支払う必要があります。ほとんどの国籍の人が初めて417ビザを申請する場合、生体認証や健康診断の追加費用はかかりません。",
          ],
        },
        {
          heading: "資金証明：AUD 5,000 + 帰国便",
          paragraphs: [
            "移民・国境保護省は、ワーキングホリデービザ保有者が滞在中の生活費として十分な資金を持つことを求めています。広く参照される基準はAUD 5,000の流動資産に加え、帰国または出国便のチケット、またはその購入資金です。認められる書類や審査での提示方法については、[ワーホリビザの資金証明ガイド](~/blog/proof-of-funds-working-holiday-visa)をご覧ください。",
            "空港で銀行口座明細書を求められることは通常ありませんが、入国審査官が要求する場合があります。より実際的には、AUD 5,000の基準は、家主や雇用主が資金証明を求めるあらゆる状況に備えて、銀行口座に示すべき金額です。",
          ],
          bullets: [
            "普通預金または取引口座にAUD 5,000 — 流動資産のみ。クレジットカードの限度額は含まれません",
            "帰国または出国便の予約、あるいは購入に充てる相当の資金",
            "氏名と残高が記載された過去3ヶ月以内の銀行口座明細書",
            "AUD 10,000以上を持ち込んだり送金したりする場合は、入国申告書に記載が必要",
          ],
        },
        {
          heading: "初月の予算内訳",
          paragraphs: [
            "オーストラリアでの最初の月は通常最も出費が多くなります — シェアハウスを見つける前の宿泊費、キッチンを使える前の外食費、持ってこなかった必需品の購入などがかかります。シドニーやメルボルンなどの大都市を拠点にする場合の週次・月次の現実的な内訳を紹介します。",
          ],
          bullets: [
            "ホステルのドミトリーベッド：週AUD 180〜320（都市と季節によって大きく異なります）",
            "食費と日用品：週AUD 80〜150（ホステルのキッチンを使えばコストを大幅に削減できます）",
            "現地SIMカード：初月AUD 30〜50（Telstra、Optus、VodafoneのプリペイドSIM）",
            "公共交通機関：週AUD 50〜100（移動量によって異なります）",
            "賃貸デポジット：シェアハウスに入居する場合AUD 1,000〜2,000（通常4週間分の家賃の前払いとデポジット）",
            "雑費（日用品、衣類、娯楽）：月AUD 100〜200",
          ],
        },
        {
          heading: "初給料までの余裕資金",
          paragraphs: [
            "オーストラリアの雇用主は週払いまたは2週間払いが多いです。1週目に就労を開始すれば7〜14日以内に最初の給与を受け取れる可能性がありますが、給与処理の遅れ、銀行口座の開設、Tax File Number（TFN）の取得にかかる時間を考えると、2〜4週間の無収入期間は現実的な見通しです。",
            "上記のコストの4週間分を合計すると、収入ゼロの状態でAUD 1,800〜3,000の生活費がかかります。だからこそAUD 5,000は最低限であり、余裕のある金額ではありません。",
          ],
        },
        {
          heading: "渡航前に貯めるべき金額の目安",
          paragraphs: [
            "すべてを合算した場合の、搭乗前の現実的な貯蓄目標をまとめます。",
          ],
          bullets: [
            "最低限（節約生活、ホステルのシェアルーム、すぐに就労開始）：AUD 5,000 + 帰国便",
            "余裕のあるバッファ（初月の生活費をカバー、小額の緊急資金）：AUD 7,000〜8,000",
            "先に旅行したい、または就職先探しに時間をかけたい場合の推奨額：AUD 10,000以上",
            "注意：これらは到着時点の貯蓄額です — 出発前に支払うビザ申請費用は含まれていません",
          ],
        },
        {
          heading: "Ternができること",
          paragraphs: [
            "Ternの資金証明書ジェネレーターを使えば、入国審査官、家主、雇用主に提示するための整ったフォーマットの残高サマリーを簡単に出力できます。渡航前に自国からリアルミッドマーケットレートでTernアカウントに入金できるので、追加費用なしでAUDを持って到着できます。就労開始後は給料日当日に振込が完了し、Ternは一切ATM手数料を請求しません — 引き出した金額はすべてあなたのものです。",
          ],
        },
      ],
      faq: [
        {
          q: "オーストラリアのワーキングホリデーに必要なお金はいくらですか？",
          a: "移民・国境保護省はAUD 5,000の流動資産プラス出国または帰国便を基準としています。さらに、渡航前に支払うAUD 670程度のビザ申請費用と、最初の給与が入るまでの約AUD 1,800〜3,000の初月生活費を予算に含めてください。渡航前の貯蓄目標をAUD 7,000〜8,000とすれば十分な余裕が生まれます。",
        },
        {
          q: "オーストラリアの入国審査で実際に資金確認がありますか？",
          a: "オーストラリアの国境職員は確認することがあり、移民・国境保護省は申請時や入国時に証明書類を求める場合があります。より一般的には、家主、シェアハウス募集、一部の雇用主が銀行口座明細書を求めます。銀行口座（クレジット限度額ではない）にAUD 5,000が見えていることが実用的な基準です。",
        },
        {
          q: "自国の銀行口座で資金証明として使えますか？",
          a: "はい — 自国の口座でAUD 5,000相当の残高を示す銀行口座明細書は、ビザ申請と入国審査の両方で受け入れられます。ただし、オーストラリアに到着したら雇用主からの直接振込を受けるためのオーストラリアの銀行口座（BSBと口座番号付き）が必要です。到着したらすぐに口座を開設し、同時にTax File Numberを申請しましょう。",
        },
      ],
    },
    ko: {
      title: "호주 워킹홀리데이에 필요한 돈은 얼마?",
      description:
        "호주 417 비자 신청 비용, AUD 5,000 자금 증명, 첫 달 예산 내역, 출발 전 저축 목표까지 정확한 수치로 알려드립니다.",
      excerpt:
        "비자는 약 AUD 670입니다. 은행에 AUD 5,000과 귀국편이 필요합니다. 호주에서 첫 달에 실제로 드는 비용과 충분한 여유를 갖고 도착하는 방법을 알아보세요.",
      sections: [
        {
          paragraphs: [
            "호주 워킹홀리데이 비자(서브클래스 417)는 세계에서 가장 인기 있는 프로그램 중 하나이지만, 항공권을 예약하기 전에 은행에 얼마가 있어야 하는지 정확히 알아야 합니다. 비자 비용, 자금 증명 요건, 첫 달 실제 비용을 합치면 대부분이 예상하는 것보다 큰 금액이 됩니다.",
          ],
        },
        {
          heading: "비자 신청 비용",
          paragraphs: [
            "서브클래스 417 워킹홀리데이 비자의 현재 신청 비용은 약 AUD 670입니다(2026년 중반 기준 — 비용은 매년 검토되므로 신청 전 반드시 호주 이민부 웹사이트에서 최신 금액을 확인하세요). 비용은 환불 불가이며 ImmiAccount를 통해 신청 시 온라인으로 결제해야 합니다. 대부분의 국적자가 처음 417 비자를 신청할 때는 별도의 생체 인식이나 건강검진 비용이 없습니다.",
          ],
        },
        {
          heading: "자금 증명: AUD 5,000 + 귀국편",
          paragraphs: [
            "호주 이민부는 워킹홀리데이 비자 소지자가 체류 기간 동안 자신을 부양할 충분한 자금을 보유하도록 요구합니다. 널리 인용되는 기준은 AUD 5,000의 인출 가능한 저축금액과 귀국 또는 출국 항공권, 혹은 구매할 수 있는 자금입니다. 인정되는 서류와 제출 방법에 대한 자세한 내용은 [워홀 비자 자금 증명 가이드](~/blog/proof-of-funds-working-holiday-visa)를 참고하세요.",
            "공항에서 은행 명세서 제출을 요청받는 경우는 드물지만 입국 심사관이 요청할 수 있습니다. 더 현실적으로는, AUD 5,000 기준은 집주인이나 고용주가 자금 증명을 요구하는 상황을 포함해 어떤 시나리오에도 대비하기 위해 은행 계좌에 표시되어야 할 금액입니다.",
          ],
          bullets: [
            "저축 또는 입출금 계좌에 AUD 5,000 — 유동 자금만 해당. 신용카드 한도는 포함되지 않음",
            "귀국 또는 출국 항공권 예약, 혹은 이를 구매할 수 있는 동등한 자금",
            "이름과 잔액이 표시된 최근 3개월 이내 은행 명세서",
            "AUD 10,000 이상을 携帶하거나 송금하는 경우 입국 신고서에 신고 필요",
          ],
        },
        {
          heading: "첫 달 예산 내역",
          paragraphs: [
            "호주에서의 첫 달은 보통 비용이 가장 많이 드는 시기입니다 — 쉐어하우스를 찾기 전 숙박비, 부엌을 쓸 수 있기 전 외식비, 미처 챙기지 못한 물품 구매 등이 발생합니다. 시드니나 멜버른 같은 대도시에 머무르는 경우의 현실적인 주간 및 월간 내역입니다.",
          ],
          bullets: [
            "호스텔 도미토리 침대: 주당 AUD 180–320 (도시와 계절에 따라 크게 다름)",
            "식비 및 장보기: 주당 AUD 80–150 (호스텔 공용 주방 이용 시 비용이 크게 줄어듦)",
            "현지 SIM 카드: 첫 달 AUD 30–50 (Telstra, Optus, Vodafone 선불 요금제)",
            "대중교통: 주당 AUD 50–100 (이동 빈도에 따라 다름)",
            "임대 보증금: 쉐어하우스 입주 시 AUD 1,000–2,000 (보통 4주치 선납 임대료 + 보증금)",
            "기타 비용 (생필품, 의류, 여가): 월 AUD 100–200",
          ],
        },
        {
          heading: "첫 월급까지의 여유 자금",
          paragraphs: [
            "호주 고용주 대부분은 주급 또는 격주급으로 지급합니다. 첫 주에 취업하면 7~14일 내에 첫 급여를 받을 수 있지만, 급여 처리 지연과 은행 계좌 개설 및 Tax File Number(TFN) 취득에 필요한 시간을 감안하면 2~4주간 수입이 없는 것이 현실적인 예상입니다.",
            "위의 비용 4주치를 합산하면 아무것도 벌지 못한 상태에서 AUD 1,800–3,000의 생활비가 발생합니다. 그래서 AUD 5,000은 최소 기준이지 여유 있는 금액이 아닙니다.",
          ],
        },
        {
          heading: "출발 전 저축 목표 요약",
          paragraphs: [
            "모든 것을 합산한 출발 전 현실적인 저축 목표입니다.",
          ],
          bullets: [
            "최소 필요액 (빠듯한 예산, 공유 호스텔, 빠른 취업): AUD 5,000 + 귀국편",
            "여유 있는 버퍼 (첫 달 생활비 커버, 소액 비상금): AUD 7,000–8,000",
            "먼저 여행하거나 일자리 찾는 시간이 필요한 경우 권장: AUD 10,000 이상",
            "주의: 이 금액은 도착 시점의 저축액 — 출국 전 납부하는 비자 신청 비용은 별도",
          ],
        },
        {
          heading: "Tern이 도와드립니다",
          paragraphs: [
            "Tern의 자금 증명 명세서 생성기를 사용하면 입국 심사관, 집주인, 고용주에게 제출할 깔끔하고 날짜가 찍힌 잔액 요약서를 간편하게 출력할 수 있습니다. 출발 전 본국에서 실제 중간 환율로 수수료 없이 Tern 계좌를 충전해 AUD를 가지고 도착할 수 있습니다. 취업 후에는 급여일 당일 입금되며, Tern은 ATM 수수료를 일절 청구하지 않습니다 — 인출한 금액은 모두 내 것입니다.",
          ],
        },
      ],
      faq: [
        {
          q: "호주 워킹홀리데이에 얼마나 많은 돈이 필요한가요?",
          a: "호주 이민부는 AUD 5,000의 인출 가능한 자금과 출국 또는 귀국편을 기준으로 설정합니다. 여기에 출발 전 납부하는 약 AUD 670의 비자 신청 비용과 첫 월급이 들어오기 전까지 현실적으로 AUD 1,800–3,000의 첫 달 생활비를 예산에 포함하세요. 출발 전 저축 목표를 AUD 7,000–8,000으로 잡으면 충분한 여유가 생깁니다.",
        },
        {
          q: "호주 입국 심사에서 실제로 자금 확인을 하나요?",
          a: "호주 국경 심사관이 확인할 수 있으며, 이민부는 신청 시 또는 입국 시 증빙 서류를 요청할 수 있습니다. 더 흔하게는 집주인, 쉐어하우스 모집 공고, 일부 고용주가 은행 명세서를 요청합니다. 은행 계좌(신용 한도가 아닌)에 AUD 5,000이 표시되어 있는 것이 충족해야 할 실용적인 기준입니다.",
        },
        {
          q: "자국 은행 계좌로 자금 증명을 할 수 있나요?",
          a: "네 — 자국 계좌에 AUD 5,000 상당의 잔액을 보여주는 은행 명세서는 비자 신청과 국경 심사 모두에 적합합니다. 단, 호주에 도착하면 고용주로부터 직접 입금을 받기 위해 호주 은행 계좌(BSB 및 계좌 번호 포함)가 필요합니다. 도착하자마자 계좌를 개설하고 동시에 Tax File Number를 신청하세요.",
        },
      ],
    },
    "zh-TW": {
      title: "澳洲打工度假需要多少錢？",
      description:
        "澳洲417簽證費用、AUD 5,000資金證明、第一個月預算明細，以及出發前需要存多少錢的完整數字指南。",
      excerpt:
        "簽證費約AUD 670。銀行裡需要AUD 5,000加上回程機票。這裡告訴你在澳洲第一個月的實際花費，以及如何帶著足夠的緩衝資金抵達。",
      sections: [
        {
          paragraphs: [
            "澳洲打工度假簽證（子類別417）是全球最熱門的計畫之一——但在訂機票之前，你必須清楚知道銀行裡需要存多少錢。簽證費、資金證明要求，加上第一個月的實際生活開銷，總金額往往比大多數人預想的要高。",
          ],
        },
        {
          heading: "簽證申請費",
          paragraphs: [
            "417打工度假簽證目前的申請費約為AUD 670（截至2026年中 — 費用每年都會調整，申請前請務必至澳洲移民部官網確認最新金額）。此費用不予退還，須在透過ImmiAccount提交申請時線上繳付。大多數國籍首次申請417簽證時，不需額外支付生物識別或體檢費用。",
          ],
        },
        {
          heading: "資金證明：AUD 5,000 + 離境機票",
          paragraphs: [
            "澳洲移民部要求打工度假簽證持有者在整個居留期間具備充足的自給能力。普遍引用的基準是AUD 5,000的可動用儲蓄，加上回程或離境機票——或可用來購票的等額資金。關於哪些文件符合要求及如何出示，請參閱[打工度假簽證資金證明指南](~/blog/proof-of-funds-working-holiday-visa)。",
            "通常不會在機場被要求出示銀行對帳單，但入境官員有權要求查驗。更實際的情況是，當房東或雇主要求出示資金證明時，你的帳戶必須能顯示AUD 5,000。",
          ],
          bullets: [
            "儲蓄或交易帳戶中AUD 5,000 — 僅限流動資金；信用卡額度不計",
            "回程或離境機票預訂，或等額購票資金",
            "近3個月內開立、載有姓名與餘額的銀行對帳單",
            "若攜帶或轉入AUD 10,000以上，須於入境旅客申報卡上申報",
          ],
        },
        {
          heading: "第一個月預算明細",
          paragraphs: [
            "在澳洲的第一個月通常是花費最多的時候——在找到合租房之前你需要住旅館，在有廚房之前你只能在外用餐，還得採購沒帶來的日用品。以下是以雪梨或墨爾本等大城市為據點的現實週支出與月支出明細。",
          ],
          bullets: [
            "青年旅館床位：每週AUD 180–320（依城市和季節差異顯著）",
            "餐飲與食材：每週AUD 80–150（在旅館公共廚房自炊可大幅降低開銷）",
            "本地SIM卡：第一個月AUD 30–50（Telstra、Optus或Vodafone預付方案）",
            "大眾交通：每週AUD 50–100，依移動頻率而定",
            "租屋保證金：入住合租房須AUD 1,000–2,000（通常為4週預付租金加保證金）",
            "雜項（盥洗用品、服裝、娛樂）：每月AUD 100–200",
          ],
        },
        {
          heading: "等到第一份薪水前的緩衝資金",
          paragraphs: [
            "澳洲大多數雇主採週薪或雙週薪制。若第一週就開始工作，最快7至14天內即可收到第一筆薪資——但考量到薪資處理延遲、開立銀行帳戶及取得Tax File Number（TFN）所需的時間，二至四週零收入是合理的預期。",
            "將上述費用的四週加總，在毫無收入的情況下生活開銷約為AUD 1,800–3,000。這正是AUD 5,000只是最低標準，而非充裕緩衝的原因。",
          ],
        },
        {
          heading: "出發前需存多少錢：參考總額",
          paragraphs: [
            "綜合以上所有費用，以下是登機前現實的儲蓄目標。",
          ],
          bullets: [
            "最低限度（預算緊湊、住合住旅館、盡快就業）：AUD 5,000 + 回程機票",
            "舒適緩衝（涵蓋第一個月費用、小額緊急備用金）：AUD 7,000–8,000",
            "建議預算（想先旅遊或慢慢找工作者）：AUD 10,000以上",
            "注意：以上為抵達時的存款目標，不含出發前需支付的簽證費",
          ],
        },
        {
          heading: "Tern 如何協助你",
          paragraphs: [
            "Tern的資金證明文件生成器讓你輕鬆匯出格式整齊、附有日期的餘額摘要，供入境官員、房東或雇主查驗——不必再費心找正確格式的銀行對帳單。你可以在出發前從本國以真實中間匯率零手續費充值Tern帳戶，抵達時口袋裡就已有AUD。開始工作後，薪資發薪日當天即時到帳，Tern也不收取任何ATM手續費——每一分提款都是你的。",
          ],
        },
      ],
      faq: [
        {
          q: "澳洲打工度假需要多少錢？",
          a: "澳洲移民部將標準設為AUD 5,000的可動用資金加上出境或回程機票。此外，還需預算出發前支付的約AUD 670簽證費，以及首份薪資入帳前現實上約AUD 1,800–3,000的第一個月生活費。將出發前的儲蓄目標定為AUD 7,000–8,000，能為你提供充裕的緩衝。",
        },
        {
          q: "澳洲入境時真的會查驗資金證明嗎？",
          a: "澳洲邊境官員可能會查驗，移民部也可在申請或入境時要求出示證明文件。更常見的情況是房東、合租廣告刊登者和部分雇主會要求查看銀行對帳單。銀行帳戶（非信用額度）中顯示AUD 5,000，是你應達到的實際標準。",
        },
        {
          q: "可以用本國銀行帳戶作為資金證明嗎？",
          a: "可以——本國帳戶顯示相當於AUD 5,000的餘額對帳單，對簽證申請和入境查驗均有效。不過，到了澳洲後，你需要一個澳洲銀行帳戶（含BSB和帳號）才能接受雇主直接存款。抵達後請盡快開戶，並同步申請Tax File Number。",
        },
      ],
    },
  },
};
