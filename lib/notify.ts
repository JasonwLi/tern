...
> = {
  ...
  de: {
    ...
    hi: "Du bist dabei!",
    position: (pos) => `Du bist Nummer ${pos} auf der Warteliste von Tern, der Geld-App für Work and Travel.`,
    share: "Willst du nach vorne? Jeder Freund, der sich über deinen Link anmeldet, bringt dich 10 Plätze nach vorn:",
    status: "Deine Position jederzeit checken",
    signoff: "— Tern",
  },
  fr: {
    subject: (pos) => `Tu es sur la liste d'attente Tern — n°${pos} 🎉`,
    hi: "C'est bon, tu es sur la liste !",
    position: (pos) => `Tu es n°${pos} sur la liste d'attente de Tern, l'app d'argent pensée pour le PVT.`,
    share: "Tu veux avancer ? Chaque ami inscrit via ton lien te fait gagner 10 places :",
    status: "Vérifie ta position quand tu veux",
    signoff: "— Tern",
  },
  it: {
    subject: (pos) => `Sei in lista d'attesa Tern — n. ${pos} 🎉`,
    hi: "Ci sei!",
    position: (pos) => `Sei il numero ${pos} in lista d'attesa per Tern, l'app per i soldi pensata per il working holiday.`,
    share: "Vuoi salire? Ogni amico che si iscrive col tuo link ti fa avanzare di 10 posti:",
    status: "Controlla la tua posizione quando vuoi",
    signoff: "— Tern",
  },
  ja: {
